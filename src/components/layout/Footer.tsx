			...
			<div className='max-w-7xl mx-auto'>
				<div className='grid grid-cols-1 md:grid-cols-3 gap-10 pb-12 border-b border-white/10 mb-10'>
					...
					<div>
						...
						>
							<img
								src='/images/shibayama_logo_lockup_white.svg'
					...
					{/* Pages */}
					<div>
						<p className='text-[12px] tracking-[0.2em] uppercase text-white/75 mb-5'>Pages</p>
						<ul className='list-none flex flex-col gap-3'>
							{pages.map(({ to, label }) => (
								<li key={to}>
									<Link
										to={to}
										className='text-[14px] text-white/90 no-underline hover:text-white transition-colors'
									>
										{label}
									</Link>
								</li>
					...
					{/* Connect */}
					<div>
						<p className='text-[12px] tracking-[0.2em] uppercase text-white/75 mb-5'>Connect</p>
						<ul className='list-none flex flex-col gap-3'>
							<li>
								<a
									href='mailto:chihiro@chihiroshibayama.com'
									className='text-[14px] text-white/90 no-underline hover:text-white transition-colors'
								>
					...
					<div className='flex gap-6'>
						{social.map(({ href, label }) => (
							<a
								...
								target='_blank'
								rel='noopener noreferrer'
								className='text-[12px] tracking-[0.1em] uppercase text-white/70 no-underline hover:text-white transition-colors'
							>
								{label}
							</a>
						))}
