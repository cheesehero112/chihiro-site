  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5">
      ...
      <button type="submit" disabled={state.submitting}
        className="bg-black text-white py-4 text-[15px] tracking-[0.2em] uppercase font-normal hover:opacity-75 transition-opacity disabled:opacity-50 cursor-pointer border-none">
    ...
    <div className="pt-[80px]">
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 min-h-[calc(90vh-80px)]">
        <div className="flex flex-col justify-center px-6 lg:px-16 py-16 lg:py-24 border-b md:border-b-0 md:border-r border-light-gray order-2 md:order-1">
          ...
          <div className="flex flex-wrap gap-4">
            ...
            <a href="#lessons" className="text-[15px] tracking-[0.15em] uppercase text-mid-gray no-underline border-b border-light-gray pb-px">
              View options ↓
            </a>
          </div>
        </div>
        ...
        <div className="relative bg-off-white overflow-hidden min-h-[85vw] md:min-h-0 order-1 md:order-2">
          ...
          <div className="absolute bottom-8 left-8 bg-black/60 px-5 py-4 border-l-[3px] border-accent max-w-[240px]">
            <p className="font-serif italic text-[15px] text-white leading-snug mb-1.5">"…impressive soloist…"</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-px bg-light-gray border border-light-gray">
            {whyReasons.map(({ num, title, text }) => (
              <div key={num} className="bg-off-white px-8 lg:px-10 py-10 relative">
                ...
                <p className="text-[15px] text-text-gray font-light leading-relaxed">{text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="px-6 lg:px-16 py-16 lg:py-24 border-b border-light-gray">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-[16px] text-text-gray font-light leading-relaxed mt-8 max-w-2xl">
...
function FAQItem({ question, answer }: { question: string; answer: string }) {
  return (
    <details className="group border-b border-light-gray">
      <summary className="flex justify-between items-center py-5 cursor-pointer list-none text-[17px] text-black font-normal">
        {question}
        <span className="text-mid-gray text-xl font-light ml-4 group-open:rotate-45 transition-transform flex-shrink-0">+</span>
      </summary>
      <p className="text-[16px] text-text-gray font-light leading-relaxed pb-5">{answer}</p>
    </details>
  )
}
